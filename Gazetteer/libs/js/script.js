        ...
        success:function(response){  
          ...
          $.ajax({
            ...
            success: function(data){
              ...
              const result = data.features[i].properties.iso_a2;  
              const lower = result.toLowerCase();
              let outline = data.features[i];
          ...
          $.ajax({
            ...
            success: function(result){
              ...
              for (let i = 0; i < 4; i++){
                ...
                const author = result.articles[i].author
                const title = result.articles[i].title
            ...
            $.ajax({
              ...
              success: function(result){
                ...
                for (let i = 0; i < result.length; i++){
                  if (result[i].CapitalName === capital){
                    const capitalLat = result[i].CapitalLatitude;
                    const capitalLng = result[i].CapitalLongitude;
            ...
            success: function(result){  
              ...
              $.ajax({
                ...
                success: function(result){
                  ...
                  for (let i = 0; i < result.length; i++){
                    if (result[i].CapitalName === capital){
                      const lat = result[i].CapitalLatitude
                      const lng = result[i].CapitalLongitude;
                      ...
                      marker.bindPopup(
                        ...
                        "</br>Country: <span class=txtcountryName></span>" +
                        "</br>Capital: <span class=txtcapital></span>").openPopup();
                      ...
                      $.ajax({
                        ...
                        dataType: 'json',
                          success: function(response){      
                            $('.continent').html(response['results'][0]['components']['continent']);
                            $('.countryCode').html(response['results'][0]['components']['country_code'])  
                        },
                        error:function(error){
                          console.log(error)
                        }
